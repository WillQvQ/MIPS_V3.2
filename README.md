...
我用直接相连的方式实现了可读写的数据缓存dcache，用LRU两路组相连的方式实现了只读的指令缓存icache，并用串口调试的方式完美地展示了cache操作的全过程。我还比对了使用缓存和不使用缓存的系统在运行同一代码上的时间差距，详见<五、仿真对比>。
...
**比较替换模块（/source/compare/）**

比较替换模块用于替换top.sv和mem.sv后对测试代码进行仿真，比较是否使用缓存对处理器效率的影响

```
top1.sv				没有使用缓存，但有延迟的顶层模块
mem1.sv				没有使用缓存，但有延迟的内存
top2.sv				只使用指令缓存的顶层模块
mem2.sv				只使用指令缓存的内存
```
